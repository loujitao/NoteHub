...
![img](./images/断路器配置1.png) 

![img](./images/断路器配置2.png) 

![img](./images/断路器配置3.png) 

![img](./images/断路器配置4.png) 

#### > hystrix工作流程

![img](./images/hystrix工作流程.png)  
...
<img src="./images/gateway工作流程.png"> 
...
&emsp;使用 `@GlobalTransactional` 注解， 高效并且对业务零侵入地解决分布式事务问题。。
...
&emsp;http://localhost:9001/payment/nacos/1
...
- 配置 yaml 文件
  <img src="./images/nacos-config1.png"> 
   <img src="./images/nacos-config2.png">  
...
1. Nacos默认的命名空间是 public ，Namespace主要实现隔离。
   比如说现在有三个环境：开发(dev)，测试(test)，生产环境(prod)，就可以创建三个Namespace，不同的Namespace之间是隔离的。
...
&emsp;指定 spring.profile.active 与配置文件的DataID来使不同环境下读取不同的配置
默认空间+默认分组+新建dev和test两个DataID。

1. Nacos配置中心新建`nacos-config-client-test.yaml`
...
&emsp;如同包一样 Namespace > Group > DataID
...
参考：https://www.cnblogs.com/linchenguang/p/12827582.html

<img src="./images/nacos集群.jpeg">  

翻译上面这张图：

![img](./images/nacos集群.PNG) 

**前提配置**

&emsp;**1个Nginx + 3个nacos注册中心 + 1个mysql**

**linux安装mysql** 

1. 下载：``wget https://dev.mysql.com/get/Downloads/MySQL-5.7/mysql-5.7.24-linux-glibc2.12-x86_64.tar.gz``
2. 将安装包放到指定目录下，解压`tar xzvf mysql-5.7.24-linux-glibc2.12-x86_64.tar.gz`
3. 移动到 /usr/local目录下
...
2. 修改`nacos\conf\application.properties`文件，增加支持mysql数据源配置（目前只支持mysql），添加mysql数据源的url、用户名和密码。
...
1. 复制`nacos/config`下的 `cluster.config.example` 重命名为 `cluster.config`
2. ip地址查看 `hostname -i`
3. **修改conf/cluster.config文件内容**
...
3. **修改 bin 下的 startup.sh, 使它能够接受不同的启动端口** 

> 集群启动，希望传递不同的端口号，启动不同的nacos实例
>
> 例如：./startup.sh -p 3333
...
http://martinhub.gitee.io/martinhub-notes/#/./notes/04-Linux%E7%9B%B8%E5%85%B3/03-Nginx/README?id=%e4%b8%80%e3%80%81nginx-%e7%bc%96%e8%af%91%e6%96%b9%e5%bc%8f%e5%ae%89%e8%a3%85

命令： `cd /opt/software/nginx/sbin`
...
5. **配置nginx，使它作为负载均衡器**
   ...
   启动nginx: `nginx -c /opt/nginx/conf/nginx.conf`
...
6. **启动3个nacos服务**
   ...
   查看启动了几个nacos:  `ps -ef | grep nacos | grep -v grep | wc -l`

   如果发先只启动了一个，那么很有可能是因为内存不够，此时修改`startup.sh`
...
> 1. 防火墙开放端口3333,4444,5555,1111
> 2. 打开mysql服务
> 3. 有错误要去nacos的log文件夹内查询

7. **测试**

   将IDEA对应端口换成 nginx 地址即可：ip地址:1111

   浏览器访问：`ip地址:1111/nacos`
